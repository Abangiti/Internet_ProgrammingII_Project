import React from 'react'

export default function Event() {
  return (
    <main className="above">
  <h1>Recent Events:</h1>
  <div className="recent">
    <div className="card">
      <div className="card_img">
        <img
          src="../Resources/Images/Event_page/gena.jpg"
          alt="Christmas Art in Ethiopian culture"
        />
      </div>
      <div className="below_img">
        <h3>Ethiopian Christmas</h3>
        <ul className="address">
          <li>
            <img
              src="../Resources/Images/Event_page/\location_on_FILL0_wght400_GRAD0_opsz24.svg"
              alt="location icon"
            />
            Lalibela, Ethiopia
          </li>
          <li>
            <img
              src="../Resources/Images/Event_page/\icons8-clock.svg"
              alt="clock icon"
            />
            Jan 7, 2:00 pm.
          </li>
          <li>
            <img
              src="../Resources/Images/Event_page/\payments_FILL0_wght400_GRAD0_opsz24.svg"
              alt="currency icon"
            />
            500 birr
          </li>
        </ul>
        <hr />
        <p className="p">
          Not Just a job to us, but a way of life! We bring the life of the
          party to you
        </p>
      </div>
    </div>
    <div className="card">
      <div className="card_img">
        <img
          src="../Resources/Images/Event_page/\spectacle.jpg "
          alt="Spectacle Events"
        />
      </div>
      <div className="below_img">
        <h3>Spectacle Events</h3>
        <ul className="address">
          <li>
            <img
              src="../Resources/Images/Event_page/location_on_FILL0_wght400_GRAD0_opsz24.svg"
              alt="location icon"
            />
            Addis Ababa, Ethiopia
          </li>
          <li>
            <img
              src="../Resources/Images/Event_page/icons8-clock.svg"
              alt="clock icon"
            />
            Jan 7, 2:00 pm.
          </li>
          <li>
            <img
              src="../Resources/Images/Event_page/payments_FILL0_wght400_GRAD0_opsz24.svg"
              alt="currency icon"
            />
            Free
          </li>
        </ul>
        <hr />
        <p className="p">
          Immersive experiences that captivate audiences and deliver
          unforgettable moments of entertainment.
        </p>
      </div>
    </div>
    <div className="card">
      <div className="card_img">
        <img
          src="../Resources/Images/Event_page/flawless.jpg"
          alt="FLAWLESS EVENTS"
        />
      </div>
      <div className="below_img">
        <h3>Flawless Events</h3>
        <ul className="address">
          <li>
            <img
              src="../Resources/Images/Event_page/location_on_FILL0_wght400_GRAD0_opsz24.svg"
              alt="location icon"
            />
            bole, Addis Ababa
          </li>
          <li>
            <img
              src="../Resources/Images/Event_page/icons8-clock.svg"
              alt="clock icon"
            />
            Feb 7, 2:00 pm.
          </li>
          <li>
            <img
              src="../Resources/Images/Event_page/payments_FILL0_wght400_GRAD0_opsz24.svg"
              alt="currency icon"
            />
            100 birr
          </li>
        </ul>
        <hr />
        <p className="p">
          Flawless events seamlessly orchestrate every detail, ensuring
          impeccable execution.
        </p>
      </div>
    </div>
  </div>
  <h1 id="v">Virtual Events:</h1>
  <div className="virtualEvents">
    <div className="card">
      <div className="card_img">
        <img
          src="../Resources/Images/Event_page/kidscode.jpg"
          alt="Kids Coding Time"
        />
      </div>
      <div className="below_img">
        <h3>Kids Coding Time</h3>
        <ul className="address">
          <li>
            <img
              src="../Resources/Images/Event_page/location_on_FILL0_wght400_GRAD0_opsz24.svg"
              alt="location icon"
            />
            Online Event
          </li>
          <li>
            <img
              src="../Resources/Images/Event_page/calendar_month_FILL0_wght400_GRAD0_opsz24.svg"
              alt="calander icon"
            />
            January 14,2023.
          </li>
          <li>
            <img
              src="../Resources/Images/Event_page/icons8-clock.svg"
              alt="clock icon"
            />
            9:00 am - 10:00 am.
          </li>
        </ul>
        <hr />
        <p className="p">
          It is an engaging and educational program designed to introduce
          children to the exciting world of computer programming.
        </p>
      </div>
    </div>
    <div className="card">
      <div className="card_img">
        <img
          src="../Resources/Images/Event_page/ict.jpg"
          alt="Innovate Ethiopia"
        />
      </div>
      <div className="below_img">
        <h3>Innovate Ethiopia</h3>
        <ul className="address">
          <li>
            <img
              src="../Resources/Images/Event_page/location_on_FILL0_wght400_GRAD0_opsz24.svg"
              alt="location icon"
            />
            Online Event
          </li>
          <li>
            <img
              src="../Resources/Images/Event_page/calendar_month_FILL0_wght400_GRAD0_opsz24.svg"
              alt="calander icon"
            />
            December 9,2022.
          </li>
          <li>
            <img
              src="../Resources/Images/Event_page/icons8-clock.svg"
              alt="clock icon"
            />
            11 am - 1 pm (EAT).
          </li>
        </ul>
        <hr />
        <p className="p">
          A dynamic and compelling event where entrepreneurs present their ideas
          and business concepts.
        </p>
      </div>
    </div>
    <div className="card">
      <div className="card_img">
        <img
          src="../Resources/Images/Event_page/codenight.jpg"
          alt="CodeNight Program"
        />
      </div>
      <div className="below_img">
        <h3>CodeNight Program</h3>
        <ul className="address">
          <li>
            <img
              src="../Resources/Images/Event_page/location_on_FILL0_wght400_GRAD0_opsz24.svg"
              alt="location icon"
            />
            Ethiopia, Online Event
          </li>
          <li>
            <img
              src="../Resources/Images/Event_page/calendar_month_FILL0_wght400_GRAD0_opsz24.svg"
              alt="calander icon"
            />
            July 31, 2022.
          </li>
          <li>
            <img
              src="../Resources/Images/Event_page/icons8-clock.svg"
              alt="clock icon"
            />
            8:00 pm - 9:00 pm.
          </li>
        </ul>
        <hr />
        <p className="p">
          An exhilarating and collaborative event where passionate coders and
          developers present their innovative projects and....
        </p>
      </div>
    </div>
  </div>
</main>

  )
}
